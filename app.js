...
/* app.post("/login", function (req, res, next) {
  ...
  passport.authenticate("local", function (err, usuario, info) {
    ...
    if (!usuario) return res.render("session/login", { info, totpC, captchaC });
  ...
  Usuario.findOne({ email: req.body.email }).then((usuario) => {
    if (!usuario)
      return res.render("session/forgotPassword", {
        info: { message: "Email no existe o incorrecto" },
      });
...
app.post("/resetPassword", async function (req, res) {
  try {
    ...
    if (!usuario) {
      return res.render("session/resetPassword", {
        errors: {
          email: { message: "No se encontró un usuario con ese email." },
        },
        usuario: new Usuario({ email: req.body.email }),
      });
    }
